...
#ifndef TEST_THRUSTERALLOCATIONMATRIX_CONTROLLER_HPP_
...
// #include "control_msgs/msg/thruster_allocation_matrix_controller_state.hpp"
...
using ControllerStateMsg = control_msgs::msg::ThrusterAllocationMatrixControllerState;
...
constexpr auto NODE_FAILURE = rclcpp_lifecycle::node_interfaces::LifecycleNodeInterface::CallbackReturn::FAILURE;
...
class TestableThrusterAllocationMatrixController
: public thruster_allocation_matrix_controller::ThrusterAllocationMatrixController
{
  FRIEND_TEST(ThrusterAllocationMatrixControllerTest, joint_names_parameter_not_set);
  FRIEND_TEST(ThrusterAllocationMatrixControllerTest, interface_parameter_not_set);
  FRIEND_TEST(ThrusterAllocationMatrixControllerTest, all_parameters_set_configure_success);
  FRIEND_TEST(ThrusterAllocationMatrixControllerTest, check_interfaces);
  FRIEND_TEST(ThrusterAllocationMatrixControllerTest, activate_success);
  FRIEND_TEST(ThrusterAllocationMatrixControllerTest, receive_message_and_publish_updated_status);
...
public:
  CallbackReturn on_init() override
  {
    ...
    return thruster_allocation_matrix_controller::ThrusterAllocationMatrixController::on_init();
  }

  CallbackReturn on_configure(const rclcpp_lifecycle::State & previous_state) override
  {
    auto ret = thruster_allocation_matrix_controller::ThrusterAllocationMatrixController::on_configure(previous_state);
  ...
  bool wait_for_commands(
    rclcpp::Executor & executor, const std::chrono::milliseconds & timeout = std::chrono::milliseconds{500})
  {
...
class ThrusterAllocationMatrixControllerTest : public ::testing::Test
{
public:
  ...
  void SetUp()
  {
    ...
    controller_ = std::make_unique<TestableThrusterAllocationMatrixController>();

    command_publisher_node_ = std::make_shared<rclcpp::Node>("command_publisher");
    force_command_publisher_ = command_publisher_node_->create_publisher<ControllerCommandWrenchMsg>(
      "/test_thruster_allocation_matrix_controller/force_references", rclcpp::SystemDefaultsQoS());
    // pose_command_publisher_ =command_publisher_node_->create_publisher<ControllerCommandPoseMsg>(
    //    "/test_thruster_allocation_matrix_controller/pose_commands", rclcpp::SystemDefaultsQoS());
    joint_command_publisher_ = command_publisher_node_->create_publisher<ControllerCommandJointMsg>(
      "/test_thruster_allocation_matrix_controller/joint_references", rclcpp::SystemDefaultsQoS());
...
protected:
  ...
  controller_interface::return_type SetUpController(
    const std::string & controller_name = "test_thruster_allocation_matrix_controller")
  {
    auto options =
      rclcpp::NodeOptions().allow_undeclared_parameters(false).automatically_declare_parameters_from_overrides(false);
    return SetUpControllerCommon(controller_name, options);
  }
  ...
  void subscribe_and_get_messages(ControllerStateMsg & msg)
  {
    ...
    auto subscription = test_subscription_node_->create_subscription<ControllerStateMsg>(
      "/test_thruster_allocation_matrix_controller/status", 10, subs_callback);
  ...
  void publish_commands()
  {
    ...
    //     if (controller_->thruster_allocation_matrix_->unified_mode_) {
...
protected:
  ...
  std::array<bool, 6> thruster_allocation_matrix_selected_axes_ = {true, true, true, true, true, true};
  std::array<double, 6> thruster_allocation_matrix_mass_ = {5.5, 6.6, 7.7, 8.8, 9.9, 10.10};
  std::array<double, 6> thruster_allocation_matrix_damping_ratio_ = {2.828427, 2.828427, 2.828427,
                                                                     2.828427, 2.828427, 2.828427};
  std::array<double, 6> thruster_allocation_matrix_stiffness_ = {214.1, 214.2, 214.3, 214.4, 214.5, 214.6};
  ...
  std::unique_ptr<TestableThrusterAllocationMatrixController> controller_;
...
class ThrusterAllocationMatrixControllerTestParameterizedMissingParameters
: public ThrusterAllocationMatrixControllerTest,
  public ::testing::WithParamInterface<std::string>
{
public:
  virtual void SetUp()
  {
    ThrusterAllocationMatrixControllerTest::SetUp();
    auto node = std::make_shared<rclcpp::Node>("test_thruster_allocation_matrix_controller");
    overrides_ = node->get_node_parameters_interface()->get_parameter_overrides();
  }

  static void TearDownTestCase() { ThrusterAllocationMatrixControllerTest::TearDownTestCase(); }
...
protected:
  controller_interface::return_type SetUpController(const std::string & remove_name)
  {
    ...
    return ThrusterAllocationMatrixControllerTest::SetUpController(
      "test_thruster_allocation_matrix_controller_no_overrides", parameter_overrides);
  }
...
// From the tutorial: https://www.sandordargo.com/blog/2019/04/24/parameterized-testing-with-gtest
class ThrusterAllocationMatrixControllerTestParameterizedInvalidParameters
: public ThrusterAllocationMatrixControllerTest,
  public ::testing::WithParamInterface<std::tuple<std::string, rclcpp::ParameterValue>>
{
public:
  virtual void SetUp() { ThrusterAllocationMatrixControllerTest::SetUp(); }
...
protected:
  controller_interface::return_type SetUpController()
  {
    ...
    parameter_overrides.push_back(param);
    return ThrusterAllocationMatrixControllerTest::SetUpController(
      "test_thruster_allocation_matrix_controller", parameter_overrides);
  }
};

#endif  // TEST_THRUSTERALLOCATIONMATRIX_CONTROLLER_HPP_
